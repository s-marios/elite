...
OBJ_PTR createBasicObject(char *eoj)
{
	...
	addProperty(base, createDataProperty(0x80, E_READ | E_NOTIFY, 1, 1, "\x30"));
	addProperty(base, createDataProperty(0x81, E_READ | E_WRITE | E_NOTIFY, 1, 1, NULL));
	addProperty(base, createDataProperty(0x82, E_READ, 4, 4, "\0\0H\0"));
	addProperty(base, createDataProperty(0x88, E_READ | E_NOTIFY, 1, 1, "\x42"));
...
OBJ_PTR createNodeProfileObject()
{
	...
	//class properties
	addProperty(node, createDataProperty(0x80, E_READ | E_NOTIFY, 1, 1, "\x30"));
	addProperty(node, createDataProperty(0x82, E_READ, 4, 4, "\x01\0x0C\0x01\x00"));
	addProperty(node, createDataProperty(0x83, E_READ, 17, 17,
	...
	addProperty(node, createDataProperty(0xD5, E_READ | E_NOTIFY, E_INSTANCELISTSIZE, 0,
	        NULL));
	addProperty(node, createDataProperty(0xD6, E_READ | E_NOTIFY, E_INSTANCELISTSIZE, 0,
	        NULL));
...
int computeClasses(OBJ_PTR oHead, unsigned char *result)
{
	...
	FOREACH(oHead, OBJ_PTR) {
		if (!isClassPresent(result, element->eoj)) {
			if (result[0] >= 8) {
				PPRINTF( "compute classes: out of class list \
	...
	FOREACH(oHead, OBJ_PTR) {
		if (!isInstancePresent(result, element->eoj)) {
			if (result[0] >= (maxinstances)) {
				PPRINTF( "compute instances: out of space, \
						ignoring instance\r\n");
			} else {
				//check for node profile (exclude)
				if (CMPEOJ(element->eoj, PROFILEEOJ)) {
